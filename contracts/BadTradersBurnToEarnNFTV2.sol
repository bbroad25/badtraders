...
/**
 * @title BadTradersBurnToEarnNFTV2
 * @notice ERC-721 NFT contract that requires burning BadTraders tokens to mint
 * @dev Version 2 of the Burn To Earn NFT series
 *
 * Version Specifications:
 * - V1: Burn 10M tokens to mint, Max Supply: 100 NFTs
 * - V2: Burn 25M tokens to mint, Max Supply: 900 NFTs (this contract)
 *
 * Features:
...
contract BadTradersBurnToEarnNFTV2 is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, IERC4906, ReentrancyGuardUpgradeable {
    ...
    uint256 public burnAmount; // Amount of tokens to burn (V1: 10M tokens, V2: 25M tokens)
    uint256 public maxSupply; // Maximum number of NFTs that can be minted (V1: 100, V2: 900)
    ...
    // Events
    event BadTradersBurnToEarnNFTV2Minted(
        address indexed to,
     ...
     * @param _burnAmount Amount of tokens to burn (V1: 10M tokens = 10_000_000 * 10^18, V2: 25M tokens = 25_000_000 * 10^18)
     * @param _maxSupply Maximum number of NFTs that can be minted (V1: 100, V2: 900)
     */
    function initialize(
        ...
        __ERC721_init("BadTraders Burn To Earn NFT V2", "BTB2E-V2");
    ...
    function mint(string memory imageUrl, string memory metadataJSON) external nonReentrant {
        ...
        emit BadTradersBurnToEarnNFTV2Minted(msg.sender, tokenId, imageUrl, burnAmount);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Fallback: generate basic metadata with mint number
        string memory imageUrl = _tokenImageUrl[tokenId];
        return string(abi.encodePacked(
            '{"name":"BadTraders Burn To Earn NFT V2 #',
            _toString(tokenId),
            '","description":"BadTraders Burn To Earn NFT V2 minted by burning 25M tokens","image":"',
            imageUrl,
            '","attributes":[{"trait_type":"Type","value":"BadTraders Burn To Earn NFT V2"},{"trait_type":"Series","value":"V2"},{"trait_type":"Mint Number","value":',
            _toString(tokenId),
    ...
    /**
     * @dev Get remaining supply (public view, for informational purposes)
     */
