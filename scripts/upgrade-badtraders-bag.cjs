...
async function main() {
  console.log("Upgrading BadTradersBag contract...");
  ...
  // Upgrade the proxy
  const upgraded = await upgrades.upgradeProxy(proxyAddress, BadTradersBag);
  ...
  const contract = await ethers.getContractAt("BadTradersBag", proxyAddress);
  console.log("\n📝 Contract Details (after upgrade):");
  console.log("  Name:", await contract.name());
  console.log("  Symbol:", await contract.symbol());
  console.log("  BadTraders Token:", await contract.badTradersToken());
  console.log("  Minimum Balance:", ethers.formatUnits(await contract.minimumBalance(), 18), "tokens");
  ...
  if (fs.existsSync(deploymentOutput)) {
    ...
    console.log("\n✅ Updated deployment-output-bag.json");
  }
