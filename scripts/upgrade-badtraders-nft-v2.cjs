...
async function main() {
  console.log("Upgrading BadTradersBurnToEarnNFTV2 contract...");

  // Get proxy address from deployment output or environment
  const deploymentOutput = "deployment-output-v2.json";
  let proxyAddress;
  ...
  if (fs.existsSync(deploymentOutput)) {
    ...
    proxyAddress = process.env.BADTRADERS_BURN_TO_EARN_NFT_V2_PROXY_ADDRESS;
    if (!proxyAddress) {
      console.error("\n❌ ERROR: Could not find proxy address!");
      console.error("   Either provide deployment-output-v2.json or set BADTRADERS_BURN_TO_EARN_NFT_V2_PROXY_ADDRESS in .env");
      process.exit(1);
    }
  }
  ...
  // Upgrade the proxy
  const upgraded = await upgrades.upgradeProxy(proxyAddress, BadTradersNFTV2);
  ...
  const contract = await ethers.getContractAt("BadTradersBurnToEarnNFTV2", proxyAddress);
  ...
  if (fs.existsSync(deploymentOutput)) {
    ...
    console.log("\n✅ Updated deployment-output-v2.json");
  }
